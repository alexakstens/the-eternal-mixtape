...
float* warp_audio(float* input_data, unsigned int input_frames, unsigned int channels, 
    ...
    unsigned int expected_out_len = (unsigned int)(input_frames / tempoRatio) + WIN_SIZE * 4;
    ...
    while (pIn < (int)input_frames - WIN_SIZE) {
        ...
        for (int c = 0; c < (int)channels; c++) {
            ...
            float resamp_scale = (float)WIN_SIZE / lx;
            for (int i = 0; i < lx; i++) {
                float pos = i * resamp_scale;
                int idx = (int)pos;
                float frac = pos - idx;
